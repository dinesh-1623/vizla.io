...
import { RunGroupsModal } from '../../components/zones/RunGroupsModal';
...
export default function ZoneCapacityPage() {
  ...
  const [showRunGroupsModal, setShowRunGroupsModal] = useState(false);
  const [selectedZoneForRunGroups, setSelectedZoneForRunGroups] = useState<string>('');
  ...
  const handleOpenDispatch = (zoneId: string, market: string, shift: string) => {
    toast.success(`Opening Dispatch for ${zoneId}`, {
      description: `Market: ${market} | Shift: ${shift}`,
      duration: 3000,
    });
    navigate(`/to-dispatch?market=${encodeURIComponent(market)}&zone=${encodeURIComponent(zoneId)}&shift=${shift}`);
  };

  const handleViewRunGroups = (zoneId: string) => {
    setSelectedZoneForRunGroups(zoneId);
    setShowRunGroupsModal(true);
  };
  ...
  return (
    <div className="min-h-screen bg-vizla-background">
      ...
      {/* Run Groups Modal */}
      <RunGroupsModal
        isOpen={showRunGroupsModal}
        onClose={() => setShowRunGroupsModal(false)}
        zoneId={selectedZoneForRunGroups}
        zoneName={zones.find(z => z.id === selectedZoneForRunGroups)?.name || selectedZoneForRunGroups}
      />
    </div>
  );
}
